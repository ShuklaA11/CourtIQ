# CourtIQ — Technical Writeup

The detailed results, methodology, honest caveats, and per-sprint findings behind
[CourtIQ](README.md). Every number here is held-out (test games never seen in
training or λ-selection) and reproducible from the pinned provenance tuple; each
section names the script that regenerates it.

**Contents:** project log · RAPM results · win-probability model · RAPM lineup
ablation · gradient-boosted challenger · calibration · model vs. sportsbook ·
the pre-game gap · the injury edge · model vs. prediction market · reproducibility.
